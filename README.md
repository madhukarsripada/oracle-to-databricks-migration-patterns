...
| 02 | SCD Type 2 Dimension | PL/SQL package with MERGE | Delta MERGE with effective dates |
| 03 | Partition Exchange Loading | Oracle EXCHANGE PARTITION | Delta REPLACE WHERE |
| 04 | Materialized Views | FAST refresh MV | Delta Live Tables (DLT) |
| 05 | BULK COLLECT / FORALL | PL/SQL bulk operations | Spark batch read/write |
| 06 | Cursor Loops | PL/SQL FOR cursor LOOP | DataFrame operations (anti-pattern) |
| 07 | Hierarchical Queries | CONNECT BY | Recursive CTE / iterative DataFrame |
| 08 | Analytic / Window Functions | Oracle analytic functions | Spark window functions |
| 09 | ODI Knowledge Modules | ODI LKM / IKM / CKM | Databricks Workflows + DLT EXPECT |
| 10 | Change Data Capture | GoldenGate Extract/Replicat | Autoloader / DLT CDC |
| 11 | ROWNUM / ROW_NUMBER | Oracle ROWNUM | Spark row_number() with caveats |
| 12 | DECODE and Conditional Logic | DECODE / NVL / NVL2 | CASE WHEN / when().otherwise() |
